...
passport.deserializeUser(async (id: string, done: (err: any, user?: unknown | false | null) => void) => {
  try {
  ...
  passport.use(
    new GoogleStrategy(
      ...
      async (
        ...
        done: (err: any, user?: unknown, info?: unknown) => void
      ) => {
        try {
    ...
    new AppleStrategy(
      {
        ...
        keyID: APPLE_KEY_ID,
        privateKey: APPLE_PRIVATE_KEY.split('\n').join('\n'),
